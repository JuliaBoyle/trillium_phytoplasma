...
ggField_sd <- function(Model, Mesh,
  ...
  if(Groups == 1){
    ...
    Full.Projection$value <- c(inla.mesh.project(Projection, Model$summary.random$w$sd))
    
  }else{
    
    Full.Projection[,paste0("Group",1:Groups)] <-
      
      apply(matrix(Model$summary.random[[WName]]$sd, ncol = Groups), 2,
