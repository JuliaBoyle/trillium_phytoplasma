...
#Using GAMs to see if they model something cor.test doesn't capture
library(mgcv)
gam_ratio1<-gam(Ratio_one_to_repro ~ s(Freq_infected_adult, k=5) ,family=scat(link="identity"),method = "REML",data=all_data)
summary(gam_ratio1) #family=scat means its modelling a heavily tailed distribution
gam.check(gam_ratio1)
plot(gam_ratio1, residuals = TRUE, pch = 1, all.terms = TRUE, shade = TRUE, shift = coef(gam_ratio1)[1])

gam_ratio2<-gam(Ratio_veg_to_repro ~ s(Freq_infected_adult, k=5),family=scat(link="identity"),method = "REML",data=all_data)
summary(gam_ratio2) #family=scat means its modelling a heavily tailed distribution
gam.check(gam_ratio2)
plot(gam_ratio2, residuals = TRUE, pch = 1, all.terms = TRUE, shade = TRUE, shift = coef(gam_ratio2)[1])

gam_ratio3<-gam(Ratio_one_to_three ~ s(Freq_infected_adult, k=5),family=scat(link="identity"),method = "REML",data=all_data)
summary(gam_ratio3) #family=scat means its modelling a heavily tailed distribution
gam.check(gam_ratio3)
plot(gam_ratio3, residuals = TRUE, pch = 1, all.terms = TRUE, shade = TRUE, shift = coef(gam_ratio3)[1])

################
all_data<-all_data %>% mutate(Infection_presence=ifelse(infected_adult>0, "Yes", "No"))
ggplot(data=all_data, aes(Infection_presence, Ratio_one_to_three))+geom_boxplot()+theme_cowplot()+ylab("1-leaved:3-leaved ratio")+xlab("Infection presence")
ggplot(data=all_data, aes(Infection_presence, Ratio_one_to_repro))+geom_boxplot()+theme_cowplot()+ylab("1-leaved:flowering ratio")+xlab("Infection presence")
ggplot(data=all_data, aes(Infection_presence, Ratio_veg_to_repro))+geom_boxplot()+theme_cowplot()+ylab("3-leaved non-flowering:flowering ratio")+xlab("Infection presence")
